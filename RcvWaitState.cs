     ...
     class RcvWaitState : ILISState
     {
          ...
          public void RcvData(string InputString)
          {
              ...
              Console.WriteLine($"RcvData: {InputString}");
               ...
               string trimmedInput = InputString.TrimStart('\x02');
            // Encode frame number to  string that 1 becomes "\u0001"
            string frameNumberAscii = ((char)ExpectedFrame).ToString();

                
               bool isGood = trimmedInput.StartsWith(frameNumberAscii);
          ...
          private bool CheckChecksum(string InputString)
          {
            ...
            if (position < 0)
               ...
               string checkSum = message.Substring(position + 1, 2);
                string chksum = CHKSum(mainMessage);
